...
function MainController($scope, $templateRequest, $sce, $compile) {
  ...
  $scope.status = { loading: true, error: false };
  $scope.filename = BLUEPRINT_FILENAME;
  $scope.title = 'Loading...';
  $scope.registerAnchors = [];

  $templateRequest(templateUrl).then(function(blueprintHtml) {
    strippedHtml = striptags(blueprintHtml, keepTags);
    $compile($("#blueprint").html(strippedHtml).contents())($scope);
    $scope.title = BLUEPRINT_FILENAME;
    setMainPanelHeight();
    ...
    $scope.title = 'Error loading template!';
  });

  function setSidenavHeight() {
    $('#side-nav').height($(window).height() - $('#filter-box').outerHeight() - 15);
  }
  function setMainPanelHeight() {
    $('#main-panel').height($(window).height());
  }
  function setFilterBoxWidth() {
    $('#filter-box').outerWidth($('#side-nav').prop('clientWidth'));
    setSidenavHeight();
  }
  setSidenavHeight();
  setFilterBoxWidth();

  // When window resizes, reset height of main components.
  ...
  function createRegisterIndex() {
    $('a').each(function(i, el) {
      $scope.registerAnchors.push($(el).attr('name'));
    });
  }
  function fancifyTables() {
    $('table.register').each(function(i, el) {
      $(el).addClass('table table-condensed table-striped');
    });
    $('table.field').each(function(i, el) {
      $(el).addClass('table table-bordered table-hover');
    });
  }
    ...
    for (i = 0, length = html.length; i < length; i++) {
        ...
        switch (c) {
            case '<': {
                ...
                if (state == STATE_OUTPUT || state == STATE_REJECT_OUT) {
                    state = STATE_HTML;
